...
{
    FF_FIL file;
    if (FRESULT res = f_open(&file, filename, FA_OPEN_ALWAYS | FA_READ | FA_WRITE); res != FR_OK)
    {
...
bool NANDMount::CreateSaveFile(const char* path, u32 len)
{
    ...
    if (res != FR_OK)
    {
        Log(LogLevel::Error, "CreateSaveFile: failed to create file %s (%d)\n", path, res);
        return false;
    }
    ...
    // Standard TWL-SDK FAT boot sector
    data[0x000] = 0xEB;
    data[0x001] = 0x3C;
    data[0x002] = 0x90;
    ...
    *(u32*)&data[0x027] = 0x12345678;     // Volume ID
    ...
    // FAT1 begins at sector 1 (offset 0x200 = 512)
    u32 fat1Offset = sectorsize;
    if (fat1Offset + 3 <= len)
    {
    ...
    u32 fat2Offset = sectorsize * (1 + sectorsPerFat);
    if (fat2Offset + 3 <= len)
    {
