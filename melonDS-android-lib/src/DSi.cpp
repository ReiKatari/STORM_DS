...
{
    // In-memory transparent Modcrypt decryption takes precedence at ROM load stage.
    // RAM decryption is disabled to prevent double-decryption corruption.
}
...
void DSi::SetupDirectBoot()
{
    ...
    if (!dsmode)
    {
        ...
        // Note: Transparent in-memory Modcrypt decryption is handled at cart load stage.
        // Internal heuristic DecryptModcryptArea is disabled to avoid double-decryption corruption.
