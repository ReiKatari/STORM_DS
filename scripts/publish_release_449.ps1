# GitHub Release Publisher for STORM DS 4.4.9
param(
    [string]$TagName = "4.4.9",
    [string]$ReleaseName = "STORM DS 4.4.9",
    [string]$ApkPath = "E:\STORM DS\Files\STORM_DS_4.4.9.apk"
)
...
$bodyFile = Join-Path $PSScriptRoot "release_body_449.md"
...
function Upload-Asset([string]$filePath, [string]$contentType) {
    ...
    if ($existingAsset) {
        ...
        try {
            Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/releases/assets/$($existingAsset.id)" -Method Delete -Headers $headers -ErrorAction Stop
            Write-Host "Old asset deleted."
        } catch {
            Write-Warning "Could not delete asset $($existingAsset.id): $_"
        }
    }
    ...
    $curlOut = & curl.exe -s -X POST `
        -H "Authorization: Bearer $token" `
        -H "Content-Type: $contentType" `
        --data-binary "@$filePath" `
        $uploadUri

    $uploaded = $curlOut | ConvertFrom-Json
    if ($uploaded -and $uploaded.id) {
        Write-Host "Uploaded $name successfully! Size: $($uploaded.size) bytes"
    } else {
        Write-Error "Failed to upload $name. Details: $curlOut"
    }
}

Upload-Asset -filePath $ApkPath -contentType "application/vnd.android.package-archive"

Write-Host "STORM DS 4.4.9 Release process completed successfully!"
